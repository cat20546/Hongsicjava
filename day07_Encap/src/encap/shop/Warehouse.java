...
/**
 * 매장에서 판매되는 제품들을 모아두는 창고를 구현하는 클래스 제품 정보를 저장하기 위하여 배열로 관리한다.
 * ------------------------------------------- 제품들이 올려져있거나 쌓여있는 창고의 선반은 각 창고마다
 * 존재하기 때문에 구현은 멤버변수(실체변수)로 선언해야 한다. -------------------------------------------
 * 메소드
 * 
 * set(Product product) : void ==> 창고에서 들어있는 전체 제품 목록을 출력하는 기능
 * 
 * get(Product product) : product ==> 창고에 있는 제품 1개의 정보를 가져오는 기능
 * 
 * add(Product product) : void ==> 창고에 신제품을 새로 들여놓는 기능
 * 
 * remove(Product product) :void ==> 더 이상 판매하지 않는 제품을 폐기
 * 
 * getAllProducts() : Product[] 창고에 있는 전체 제품 목록을 가져오는 기능
 * 
 * @author JACK
 *
 */
public class Warehouse {
	...
	// 3. 메소드 선언부
	/**
	 * 제품 선반 가장 마지막에 추가 제품 선반 크기를 1 증가시키면서 추가
	 * 
	 * @param product
	 */
	...
	/**
	 * 파라미터로 전달된 정보와 일치하는 제품이 있는지 검색하여 (prodCode 필드가 같으면 같은 제품으로 처리) 일치하는 제품이
	 * 있으면 그 제품정보를 리턴 없으면 null을 리턴
	 * 
	 * @param product
	 */
	...
	/**
	 * 매개변수로 입력된 제품이 배열(선반) 몇번째 윛에 존재하는지 그 인덱스를 찾는다.
	 * 
	 * @param product
	 * @return
	 */
