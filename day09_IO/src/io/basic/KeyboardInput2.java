...
/**
 * 표준 입력인 키보드 입력을 받아서 표준 출력인 모니터로 출력하는 클래스 -----------------------------------
 * 1. node steam (키보드 = System.in : InputStream) 2. filter stream (stream -->
 * reader : InputStreamReader) 1바이트 인풋스트림을 2바이트 문자로 변경 (키보드는 사실 문자 입력이므로)
 * 
 * 3. filter stream (1줄 단위 편한 작업 : BufferedReader) 2바이트 문자1개를 1줄 단위로 읽어서 스트링으로
 * 만들 수 있는 편리한 메소드를 사용한다.
 * 
 * 4. 3에서 만들어진 filter stream 객체를 사용하여 read() 작업 진행
 * 
 * 5. 읽은 내용을 표준 출력(모니터 = System.out)
 * 
 * 6. 마지막 filter stream 닫기
 * 
 * @author JACK
 *
 */
